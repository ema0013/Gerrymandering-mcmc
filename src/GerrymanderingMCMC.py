...
class GerrymanderingMCMC():
    ...
    """
    def __init__(self, graph_file, directory, cooling_period=50, rounds=50, state_name="Iowa", max_pop_percentage=0.05, verbose=False):
        ...
        # self.name = name
        # self.rank = rank
        self.max_pop_percentage = max_pop_percentage
        # self.data = []
    ...
    def __is_valid_district_plan(self, edge, mst_combined_subgraph, g):
        ...
        return pop_diff < (self.max_pop_percentage*pop_total)
        ...
        while cuttable is False:
            ...
            for edge in mst_combined_subgraph.edges:
                ...
                if (attempt_count == 2000):
                    print("WARNING: Failed to make a recom after > 2000 iterations") if self.verbose else None
                    return graph
                else:
                    (tail, head) = edge
                    mst_combined_subgraph.add_edge(tail, head)
...
#        graph = self.g
#        graph_dict = {"districts": {}}
#        for node in graph.nodes(data=True):
#            try:
#                graph_dict['districts'][node[1]['district']].append(node[0])
#            except KeyError:
#                # append new district to dict
#                graph_dict['districts'][node[1]['district']] = [node[0]]
#
#                #create directory if not found
#
#        with open("output/"+self.state_name + "/" + self.directory + "/original.json", 'w') as file:
#            file.write(json.dumps(graph_dict))
        ...
        for i in range(0, rounds):
            ...
            fname = "output/"+ self.state_name + "/" + self.directory + "/recombination_of_districts" + str(uuid.uuid4()) + '.json'
